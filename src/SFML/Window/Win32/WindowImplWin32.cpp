...
void WindowImplWin32::processEvent(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_MOUSEWHEEL:
        {
            ...
            event.mouseHighPrecisionWheel.delta = static_cast<float>(HIWORD(wParam)) / 120.f;
            event.mouseHighPrecisionWheel.x     = position.x;
            event.mouseHighPrecisionWheel.y     = position.y;
            pushEvent(event);
